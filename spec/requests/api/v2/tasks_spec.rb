...
RSpec.describe 'task API' do
  ...
  describe 'GET /tasks' do
    ...
    context 'when filter param is sent' do
      let!(:notebook_task_1) { 
        create(:task, title: 'Check if the notebook is broken', user_id: user.id) 
      }

      let!(:notebook_task_2) { 
        create(:task, title: 'Buy a new notebook', user_id: user.id) 
      }

      let!(:other_task_1) { 
        create(:task, title: 'Fix the door', user_id: user.id) 
      }

      let!(:other_task_2) { 
        create(:task, title: 'Buy a new car', user_id: user.id)
      }
      ...
      it 'returns only the tasks matching' do
        ...
        expect(returned_task_titles).to eq(
          [notebook_task_1.title, notebook_task_2.title]
        )
      end
    end
  end
  ...
  describe 'POST /tasks' do
    ...
    context 'when the params are valid' do
      ...
      it 'returns the json for the created tasks' do
